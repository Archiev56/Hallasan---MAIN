...
var options: Array[WheelOption] = []
var selection := 0

func set_ability_data(ability_list: Array[Dictionary]) -> void:
	options.clear()
	for ability in ability_list:
		var opt = WheelOption.new()
		opt.name = ability.name
		opt.type = ability.type
		opt.sprite_texture = ability.icon
		options.append(opt)
	selection = 0
	queue_redraw()
...
func _draw():
	...
	if len(options) >= 3:
		...
		draw_texture_rect_region(options[0].sprite_texture, Rect2(offset, SPRITE_SIZE), options[0].region)
