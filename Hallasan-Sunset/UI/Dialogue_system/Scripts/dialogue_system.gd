...
@onready var content: RichTextLabel = $DialogUI/RichTextLabel
...
@onready var bars_animation_player = $DialogUI/BarsAnimationPlayer
...
func set_dialog_text( _d : DialogText ) -> void:
	...
	if _d.npc_info.hero:
		portrait_sprite.position.x = 460
		#content.position.x = 144
	else:
		portrait_sprite.position.x = 184
		#content.position.x = 234
